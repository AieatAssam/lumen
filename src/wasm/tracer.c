 ...
 *
 * Compile with:
 *   emcc tracer.c -O3 -s WASM=1 -s EXPORTED_FUNCTIONS='["_init","_render","_get_pixels","_set_camera","_destroy","_malloc","_free"]' -s ALLOW_MEMORY_GROWTH=1 -s TOTAL_MEMORY=256MB -o tracer.js
 */
...
static float *g_accum = NULL;     /* 4 * width * height floats (RGB + count) */
...
static void setup_spheres(void);
static void setup_glass(void);
...
static Material* add_material(Vec3 albedo, Vec3 emission, float roughness, float ior, int type) {
...
static void setup_cornell_box(void) {
    /* Materials */
    int white = add_material(v3(0.9,0.9,0.9), v3(0,0,0), 0, 1, MAT_LAMBERTIAN) - g_materials;
    int red   = add_material(v3(0.65,0.05,0.05), v3(0,0,0), 0, 1, MAT_LAMBERTIAN) - g_materials;
    int green = add_material(v3(0.12,0.45,0.15), v3(0,0,0), 0, 1, MAT_LAMBERTIAN) - g_materials;
    int light = add_material(v3(1,1,1), v3(15,15,15), 0, 1, MAT_EMISSIVE) - g_materials;
    int metal = add_material(v3(0.9,0.85,0.8), v3(0,0,0), 0.1f, 1, MAT_METAL) - g_materials;
    int glass = add_material(v3(1,1,1), v3(0,0,0), 0, 1.5f, MAT_DIELECTRIC) - g_materials;

    /* Light on ceiling */
    add_sphere(v3(1.0f, 1.95f, -0.5f), 0.15f, light);

    /* Cornell box walls (approximated as large spheres) — floor, ceiling, back, left, right */
    float R = 50.0f;
    add_sphere(v3(0, -R-1, 0), R, white);    /* floor */
    /* Ceiling is open — light sphere instead */

    /* Back wall */
    add_sphere(v3(0, 0, -R-2), R, white);
    /* Left wall (red) */
    add_sphere(v3(-R-2, 0, 0), R, red);
    /* Right wall (green) */
    add_sphere(v3(R+2, 0, 0), R, green);
...
static void setup_spheres(void) {
    int ground = add_material(v3(0.5,0.5,0.5), v3(0,0,0), 0, 1, MAT_LAMBERTIAN) - g_materials;
    int center = add_material(v3(0.7,0.3,0.3), v3(0,0,0), 0, 1, MAT_LAMBERTIAN) - g_materials;
    int left   = add_material(v3(0.8,0.8,0.8), v3(0,0,0), 0.05f, 1, MAT_METAL) - g_materials;
    int right  = add_material(v3(0.8,0.6,0.2), v3(0,0,0), 0.3f, 1, MAT_METAL) - g_materials;
    int sun    = add_material(v3(1,1,1), v3(8,7,5), 0, 1, MAT_EMISSIVE) - g_materials;
...
static void setup_glass(void) {
    int ground = add_material(v3(0.2,0.2,0.25), v3(0,0,0), 0, 1, MAT_LAMBERTIAN) - g_materials;
    int glass1 = add_material(v3(1,1,1), v3(0,0,0), 0, 1.5f, MAT_DIELECTRIC) - g_materials;
    int glass2 = add_material(v3(1,1,1), v3(0,0,0), 0, 1.5f, MAT_DIELECTRIC) - g_materials;
    int metal  = add_material(v3(0.95,0.93,0.88), v3(0,0,0), 0.02f, 1, MAT_METAL) - g_materials;
    int light  = add_material(v3(1,1,1), v3(12,10,8), 0, 1, MAT_EMISSIVE) - g_materials;
    int red    = add_material(v3(0.85,0.15,0.15), v3(0,0,0), 0, 1, MAT_LAMBERTIAN) - g_materials;
...
static Vec3 trace(Ray *ray, unsigned int *seed, int depth) {
    ...
    Vec3 wo = v3_mul(ray->dir, -1.0f); /* outgoing direction (toward camera) */

    switch (mat->type) {
    case MAT_LAMBERTIAN: {
        /* Diffuse: sample hemisphere */
        ...
        return v3_mul(v3_mulv(brdf, Li), cos_theta * 2.0f * PI); /* 2π for hemisphere PDF */
    }
    case MAT_METAL: {
...
void init(int width, int height, int scene_id) {
    ...
    g_accum = (float*)calloc(pixel_count * 4, sizeof(float)); /* RGBA accumulation */
    if (g_output) free(g_output);
    g_output = (unsigned char*)malloc(pixel_count * 4);

    switch (scene_id) {
        case 0: setup_cornell_box(); break;
        case 1: setup_spheres(); break;
        case 2: setup_glass(); break;
    ...
    for (int y = 0; y < g_height; y++) {
        for (int x = 0; x < g_width; x++) {
            ...
            for (int s = 0; s < samples_per_pixel; s++) {
                ...
                float v = ((float)y + randf(&seed)) / (float)g_height;
...
void set_camera(float ex, float ey, float ez,
    ...
    g_total_samples = 0; /* Reset accumulation on camera move */
